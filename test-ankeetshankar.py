...
def f06(string1, string2):
    ...
    if length1 >lenght2:
        ...
        percentage = (difference/length1)*100
        print("'", string1, "'", "is longer than ", "'", string2, "'", "by", difference, "characters")
        print("'", string2, "'", "has only", percentage, "the number of chars of ", "'", string2, "'" )
        various_solutions()  # Last line in f06()

    elif length1 <lenght2:
        difference = length2 - length1
        percentage = (difference/length2)*100
        print("'", string2, "'", "is longer than ", "'", string1, "'", "by", difference, "characters")
        print("'", string1, "'", "has only", percentage, "the number of chars of ", "'", string1, "'" )
        various_solutions()  # Last line in f06()

    else:
        print ("Both the strings are equal in Length and have the same number of characters ")
        various_solutions()  # Last line in f06()
...
###############################################################################
def f08():
    sum = 8 #Since I am starting the count from 5. Hence 3 and 5 are left. Their sum will be added to the total being computed in the loop
...
def f14():
    path_for_display = os.path.abspath(os.path.split(sys.argv[0])[0])
